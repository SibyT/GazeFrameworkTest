 ...
 */
public class GP3GazeProvider implements GazeDataProvider {
	 ...
	 */
	private Socket gp3ClientSocket = null;
	 ...
	 */
	public GP3GazeProvider(String serverAddress, int serverPort) throws UnknownHostException, IOException {

		// open new socket and try to connect to the GP3 on the given ip address and port
		gp3ClientSocket = new Socket(serverAddress, serverPort);

		// initialize in- and output buffer
		outToServer = new DataOutputStream(gp3ClientSocket.getOutputStream());
		inFromServer = new BufferedReader(new InputStreamReader(gp3ClientSocket.getInputStream()));
			...
			public void run() {
				...
				while (!stopListeningThread) {
					...
					if (incomingData.startsWith("<REC TIME_TICK=")) {
						...
						GP3GazePoint gp = new GP3GazePoint((int)(pogx*screen_width), (int)(pogy*screen_height), pogv, timetick);
